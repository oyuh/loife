 ...
 *
 * Picking one closes the palette and opens the compose dialog on that day,
 * the same way Add assignment hands off to the add dialog. Anything already
 * typed comes along as the first draft, so the search field doubles as the
 * capture box.
 */
const JOURNAL_DAYS = [
  { noun: 'today', offset: 0 },
...
export function CommandPalette({
  ...
  onAddItem: () => void
  onBulkAdd: () => void
  onWriteJournal: (date: string, seed: string) => void
  open: boolean
              ...
              {JOURNAL_DAYS.map(({ noun, offset }) => {
                ...
                return (
                  <CommandItem
                    key={`log-${noun}`}
                    onSelect={() =>
                      run(() => onWriteJournal(key, query.trim()))
                    }
                    value={`log-${noun}`}
                  >
                    <CalendarPlus />
            ...
            <CommandGroup heading="Journal">
              {JOURNAL_DAYS.map(({ noun, offset }) => {
                ...
                return (
                  <CommandItem
                    key={noun}
                    onSelect={() => run(() => onWriteJournal(key, ''))}
                    value={`write ${noun}`}
                  >
                    <CalendarPlus />
